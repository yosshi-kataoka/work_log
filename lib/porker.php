...
function showDown(string $aPlayerNumberOne, string $aPlayerNumberTwo, string $bPlayerNumberOne, string $bPlayerNumberTwo)
{
  $showDown = [];
  $playerPokerNumber = [];
  $playerPokerNumber = deleteFirstStr($aPlayerNumberOne, $aPlayerNumberTwo, $bPlayerNumberOne, $bPlayerNumberTwo);
  foreach ($playerPokerNumbers as $key => $playerPokerNumber) {
  }
}

function deleteFirstStr(string ...$results): array
{
  $arrayNumber = [];
  foreach ($results as $result) {
    $arrayNumber[] = mb_substr($result, 1);
  }
  return sort($arrayNumber);
}

function isStraight(array $playerPokerNumber): string
{
  $i = 0;
  $k = 1;
  if ($playerPokerNumber[$i] + 1 ===  $playerPokerNumber[$k]) {
  }
}
